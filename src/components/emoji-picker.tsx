...
export function EmojiPicker({ onSelect, onClose, isDark }: EmojiPickerProps) {
  ...
  return (
    <div className={`absolute bottom-full left-0 mb-2 w-80 rounded-2xl shadow-2xl border overflow-hidden z-50 ${isDark ? 'bg-slate-900/95 backdrop-blur-xl border-white/10' : 'bg-white/95 backdrop-blur-xl border-slate-200'}`}>
      ...
      <div className="flex overflow-x-auto gap-0.5 p-1 border-b border-inherit">
        {Object.keys(EMOJI_CATEGORIES).map((cat) => (
          <button key={cat} onClick={() => { setActiveCategory(cat); setSearch('') }}
            className={`shrink-0 px-2 py-1 text-xs rounded-lg transition-all ${activeCategory === cat ? (isDark ? 'bg-white/10 text-white' : 'bg-slate-200 text-slate-900') : (isDark ? 'text-slate-500 hover:bg-white/5' : 'text-slate-400 hover:bg-slate-100')}`}>
            {cat.split(' ')[0]}
          </button>
        ))}
      </div>
        ...
        <div className="grid grid-cols-8 gap-1">
          {filteredEmojis.map((emoji, i) => (
            <button key={`${emoji}-${i}`} onClick={() => { onSelect(emoji); onClose() }}
              className="h-9 w-9 flex items-center justify-center text-xl hover:bg-white/10 rounded-lg transition-all active:scale-90">
              {emoji}
            </button>
          ))}
        </div>
